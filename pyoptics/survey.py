...
class Curve:
    def __init__(self,xyz,label=None,
            color=None,tips=None,linestyle=None,marker=None):
        """
        xyz: 3xn array
        """
        self.xyz=xyz
...
class MadElem:
    ...
    def set_ref(self,start):
        ...
        self.ref_middle=start.advance(length=self.length/2,angle=self.angle/2,tilt=self.tilt)
        self.ref_end=start.advance(length=self.length,angle=self.angle,tilt=self.tilt)
        return self
    ...
    def render(self,ends=True):
        ...
        cav=self.ref_start.curve_ahead(self.length/2,angle=self.angle/2,tilt=self.tilt)
        ca=Curve(cav,color='k')
        cbv=self.ref_middle.curve_ahead(self.length/2,angle=self.angle/2,tilt=self.tilt)
