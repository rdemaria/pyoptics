...
def maxharm_fftr(v1,fmin=0,fmax=0.5):
  n=float(len(v1))
  # fftmax 1
  fv1=rfft(v1)
  fv1[:int(fmin*n)]=0
...
def maxharm_lsq(v1,fa=0.0,fb=0.5):
  ns=float(len(v1))
  ff1,a11,p11=maxharm_fft(v1,fa,fb)
  x=ff1,a11,p11
  t=arange(ns)
  def ftomin(x):
    ff1,a11,p11=x
...
def fit_coupled_lsq2(v1,v2):
  ...
  def ftomin(x):
    ...
    vv1=co1+a11*cos(2*pi*ff1*t+p11)+a12*cos(2*pi*ff2*t+p12)
    vv2=co2+a21*cos(2*pi*ff1*t+p21)+a22*cos(2*pi*ff2*t+p22)
    res=sum((vv1-v1)**2)/ns/sum(v1**2) + sum((vv2-v2)**2)/ns/sum(v2**2)
    return res
  x=fmin(ftomin,x,xtol=1e-10, ftol=1e-10,disp=False)
  res=ftomin(x)
  return co1,co2,ff1,ff2,a11,a12,a21,a22,p11,p12,p21,p22,res
...
def get_decay_lin(xb):
  xb-=1.0*xb.mean()
  xh=abs(scipy.signal.hilbert(xb))
  tau=-1/polyfit(arange(len(xh)),xh,1)[0]
  return tau
...
def fit_decay_interp(v,fa,fb):
  ...
  v=j/ns+arctan(alpha+sqrt(alpha**2+1))/pi
  arg0=pi*(v-j/ns)
