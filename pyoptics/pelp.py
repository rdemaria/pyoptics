...
def pelp(time,Ii,If,A,D,R,Te,Ti):
  ...
  td=(If - il)/ R + tl - R/(2*D)
  tf=td+ R/D
  I= A/2  * (time-Ti)**2 + Ii if (Ti<=time) and (time<Te) else \
     a    * exp(b*time)       if (Te<=time) and (time<tl) else \
     R    * (time - tl)  + il if (tl<=time) and (time<td) else \
     -D/2 * (tf-time)**2 + If if (td<=time) and (time<tf) else \
     0
  return I
